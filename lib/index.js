...
s3sync.prototype.syncFiles = function(operations){
  ...
  async.each(operations, function (operation, next) {
    self.sync(operation.op, operation.filename.slice(self.config.webRoot.length), next);
  }, function(err) {
...
s3sync.prototype.sync = function(operation, done) {
  var self = this,
    , op = operation.op,
    , operation.filename.slice(self.config.webRoot.length);
